...
namespace PopupKitScopedDependencies\Wpmet\UtilityPackage\Emailkit;

use PopupKitScopedDependencies\WP_Query;
\defined('ABSPATH') || exit;
if (!\class_exists('PopupKitScopedDependencies\\Wpmet\\UtilityPackage\\Emailkit\\Emailkit')) {
    class Emailkit
    {
        ...
        public function get_plugin_status($name)
        {
            ...
            if ($this->check_installed_plugin($name)) {
                if ($this->check_activated_plugin($name)) {
                    $data['title'] = __('Activated', 'popup-builder-block');
                    $data['status'] = "activated";
                } else {
                    $data['title'] = __('Activate Now', 'popup-builder-block');
                    $data['status'] = 'installed';
                    $data['activation_url'] = $this->activation_url($name);
                }
            } else {
                $data['title'] = __('Install Now', 'popup-builder-block');
        ...
        {
            if (\class_exists('PopupKitScopedDependencies\\EmailKit\\Admin\\TemplateList') && \class_exists('PopupKitScopedDependencies\\EmailKit\\Admin\\Emails\\EmailLists') && \method_exists('PopupKitScopedDependencies\\EmailKit\\Admin\\TemplateList', 'get_templates') && \method_exists('PopupKitScopedDependencies\\EmailKit\\Admin\\Emails\\EmailLists', 'woocommerce_email')) {
                $templates = \PopupKitScopedDependencies\EmailKit\Admin\TemplateList::get_templates();
                $template_title = \PopupKitScopedDependencies\Emailkit\Admin\Emails\EmailLists::woocommerce_email($wc_template_type);

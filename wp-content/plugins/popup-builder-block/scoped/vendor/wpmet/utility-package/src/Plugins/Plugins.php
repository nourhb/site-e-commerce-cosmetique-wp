...
namespace PopupKitScopedDependencies\Wpmet\UtilityPackage\Plugins;
...
class Plugins
{
    ...
    {
        add_submenu_page($this->parent_menu_slug, $this->submenu_name, '<span style="color: #2fbf17; font-weight: bold">' . $this->submenu_name . '</span>', 'manage_options', $this->text_domain . $this->menu_slug, [$this, 'wpmet_apps_renderer']);
    }
    /**
     * Generates the menus.
     *
    ...
    {
        return add_query_arg(array('page' => $this->get_plugin_slug($pluginName)), admin_url('admin.php'));
    }
    /**
    ...
    public function wpmet_apps_renderer()
    {
        ...
        foreach ($this->plugins as $key => $plugin) {
            ...
            $plugin_status_title = isset($plugin_data['title']) ? $plugin_data['title'] : \esc_html__('Activate', 'popup-builder-block');
            ?>
            ...
            if (!empty($plugin_docs)) {
                echo \sprintf('<a target="_blank" href="%1$s" class="wpmet-onboard-tut-term--help">%2$s</a>', \esc_url($plugin_docs), \esc_html__('Read Docs', 'popup-builder-block'));
            }
            ?>
								</div>
							</label>
    ...
    public function enqueue_scripts()
    {
        ...
        echo esc_attr($this->items_per_row);
        ?>, 1fr);
			}

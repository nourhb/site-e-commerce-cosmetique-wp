...
use OptinCraft\WhichBrowser\Testrunner;
use OptinCraft\WhichBrowser\Tests;
use OptinCraft\SebastianBergmann\CodeCoverage\CodeCoverage;
use OptinCraft\SebastianBergmann\CodeCoverage\Report\Clover;
...
switch ($command) {
    ...
    case 'check':
        ...
        if (!$result) {
            echo "\x1b[0;31mTestrunner failed, please fix or rebase before building or deploying!\x1b[0m\n\n";
    ...
    case 'compare':
        $result = Testrunner::compare($files, \false);
        if (!$result) {
            echo "\x1b[0;31mTestrunner failed, please look at runner.log for the details!\x1b[0m\n\n";
            if (\in_array('show', $options)) {
                echo \file_get_contents('runner.log') . "\n\n";
                echo "Done!\n\n";
            }

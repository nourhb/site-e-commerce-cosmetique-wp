...
trait Television
{
    ...
    private function detectLgTelevision($ua)
    {
        ...
        if (preg_match('/NetCast/u', $ua) && preg_match('/SmartTV\/([0-9])/u', $ua, $match)) {
            ...
            if (intval($match[1]) < 5) {
                ...
                $this->data->os->reset(['name' => 'webOS', 'hidden' => \true]);
            }
        }
        /* WebOS */
    ...
    private function detectSettopboxes($ua)
    {
        ...
        if (preg_match('/Roku\/DVP-[0-9\.]+ \(([0-9A-Z]{2,2})[0-9]+\./u', $ua, $match)) {
            ...
            $this->data->device->manufacturer = 'Roku';
            $this->data->device->type = Constants\DeviceType::TELEVISION;
            $models = ['02' => '2 XS', '03' => 'LT', '04' => '3', '07' => 'LT', '09' => 'Streaming Stick', '29' => 'Ultra', '30' => ['TCL', '4K Roku TV'], '51' => 'Express', 'AE' => 'Express'];
            if (!empty($match[1])) {
                $model = $match[1];
                if (isset($models[$model])) {
    ...
    private function detectGenericInettvBrowser($ua)
    {
        if (preg_match('/(?:DTVNetBrowser|InettvBrowser|Hybridcast)\/[0-9\.]+[A-Z]? ?\(/u', $ua, $match)) {
            ...
            if ($found) {
                $this->data->device->identified |= Constants\Id::PATTERN;
                $data = ['0003D5' => 'Advanced Communications', '000024' => 'Connect AS', '000087' => 'Hitachi', '00A0B0' => 'I-O Data Device', '00E091' => 'LG', '0050C9' => 'Maspro Denkoh', '002692' => 'Mitsubishi', '38E08E' => 'Mitsubishi', '008045' => 'Panasonic', '00E036' => 'Pioneer', '00E064' => 'Samsung', '08001F' => 'Sharp', '00014A' => 'Sony', '000039' => 'Toshiba'];

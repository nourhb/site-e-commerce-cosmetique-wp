...
namespace OptinCraft\WhichBrowser\Analyser;
...
trait Camouflage
{
    private function &detectCamouflage()
    {
        if ($ua = $this->getHeader('User-Agent')) {
            $this->detectCamouflagedAndroidBrowser($ua)->detectCamouflagedAndroidAsusBrowser($ua)->detectCamouflagedAsSafari($ua)->detectCamouflagedAsChrome($ua);
        }
    ...
    private function &detectCamouflagedAsChrome($ua)
    {
        if ($this->data->isBrowser('Chrome')) {
            if (preg_match('/(?:Chrome|CrMo|CriOS)\//u', $ua) && !preg_match('/(?:Chrome|CrMo|CriOS)\/([0-9]{1,3}\.[0-9]\.(0\.0|[0-9]{3,4}\.[0-9]+))/u', $ua)) {
    ...
    private function &detectCamouflagedBasedOnEngines()
    {
        if (isset($this->data->engine->name) && $this->data->browser->mode != 'proxy') {
            ...
            if ($this->options->engine & Constants\EngineType::WEBKIT) {
                ...
                if (isset($this->data->browser->name) && $this->data->browser->name == 'Mobile Internet Explorer' && isset($this->data->browser->version) && $this->data->browser->version->toFloat() >= 11 && isset($this->data->os->name) && $this->data->os->name == 'Windows Phone') {
                    $this->data->camouflage = \false;
                }
                /* IE 11 Developer Preview now supports  Webkit APIs */
                if (isset($this->data->browser->name) && $this->data->browser->name == 'Internet Explorer' && isset($this->data->browser->version) && $this->data->browser->version->toFloat() >= 11 && isset($this->data->os->name) && $this->data->os->name == 'Windows') {
                    $this->data->camouflage = \false;
                }

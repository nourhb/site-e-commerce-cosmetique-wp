...
use OptinCraft\WhichBrowser\Model\Device;
class DeviceModels
{
    ...
    public static function identifyIOS($model)
    {
        ...
        $model = preg_replace("/iPh([0-9],[0-9])/", 'OptinCraft\iPhone\1', $model);
        $model = preg_replace("/iPd([0-9],[0-9])/", 'OptinCraft\iPod\1', $model);
    ...
    public static function identifyAndroid($model)
    {
        ...
        if (!$result->identified) {
            ...
            if (preg_match('/AndroVM/iu', $model) || $model == 'Emulator' || $model == 'x86 Emulator' || $model == 'x86 VirtualBox' || $model == 'vm') {
                return new Device(['type' => Constants\DeviceType::EMULATOR, 'identified' => Constants\Id::PATTERN, 'manufacturer' => null, 'model' => null, 'generic' => \false]);
            }
        }
        return $result;
    }
    public static function identifyList(&$index, &$list, $model, $cleanup = \true)
    {
        $original = $model;
        if ($cleanup) {
            $model = self::cleanup($model);

...
namespace OptinCraft\Laravel\SerializableClosure\Support;
...
class ReflectionClosure extends ReflectionFunction
{
    ...
    public function getCode()
    {
        ...
        $_class = var_export(trim($className ?: '', '\\'), \true);
        ...
        for ($i = 0, $l = count($tokens); $i < $l; $i++) {
            ...
            switch ($state) {
                ...
                case 'id_name':
                    switch ($token[0]) {
                        case $token[0] === ':' && !in_array($context, ['instanceof', 'new'], \true):
        ...
        $attributesCode = array_map(function ($attribute) {
            ...
            $arguments = implode(', ', array_map(function ($argument, $key) {
                $argument = var_export($argument, \true);
                if (is_string($key)) {
    ...
    protected function fetchItems()
    {
        ...
        foreach ($tokens as $token) {
            ...
            switch ($state) {
                case 'start':
                    switch ($token[0]) {
                        case \T_NAMESPACE:
                            $structures[] = ['type' => 'namespace', 'name' => $namespace, 'start' => $namespaceStartLine, 'end' => $token[2] - 1, 'classes' => $namespaceClasses];
                            $namespace = '';
                            $namespaceClasses = [];
                            $state = 'namespace';
                        ...
                        case '}':
                            if ($namespaceBraced) {
                                $structures[] = ['type' => 'namespace', 'name' => $namespace, 'start' => $namespaceStartLine, 'end' => $lastKnownLine, 'classes' => $namespaceClasses];
                                $namespaceBraced = \false;
                                $namespace = '';
                                $namespaceClasses = [];
                            }
                ...
                case 'structure':
                    switch ($token[0]) {
                        ...
                        case '}':
                            if (--$open == 0) {
                                if (!$structIgnore) {
                                    $structures[] = ['type' => $structType, 'name' => $structName, 'start' => $startLine, 'end' => $lastKnownLine];
                                }
                                $structIgnore = \false;
                                $state = 'start';
                            }
        ...
        $structures[] = ['type' => 'namespace', 'name' => $namespace, 'start' => $namespaceStartLine, 'end' => \PHP_INT_MAX, 'classes' => $namespaceClasses];
    ...
    protected function getClosureNamespaceName()
    {
        ...
        $endLine = $this->getEndLine();
        foreach ($this->getStructures() as $struct) {
            if ($struct['type'] === 'namespace' && $struct['start'] <= $startLine && $struct['end'] >= $endLine) {
                return $struct['name'];
            }
        }
        return '';

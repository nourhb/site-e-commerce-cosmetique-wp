...
class License
{
    ...
    {
        $params = ['license_key' => $license_key, 'url' => esc_url(home_url()), 'is_local' => $this->client->is_local_server()];
        $response = $this->client->send_request($params, $route, \true);
        ...
        if (isset($response['errors']) && isset($response['errors']['license_key'])) {
            $response = ['success' => \false, 'error' => $response['errors']['license_key'][0]];
        }
        return $response;
    }
    /**
    ...
    public function refresh_license_api()
    {
        ...
        wp_send_json_success(['message' => 'License refreshed successfully.'], 200);
    }
    /**
    ...
    public function menu_output()
    {
        ...
        if (isset($_POST['_nonce']) && wp_verify_nonce(sanitize_key(wp_unslash($_POST['_nonce'])), $this->client->name)) {
            $form_data = ['_nonce' => sanitize_key(wp_unslash($_POST['_nonce'])), '_action' => isset($_POST['_action']) ? sanitize_text_field(wp_unslash($_POST['_action'])) : '', 'license_key' => isset($_POST['license_key']) ? sanitize_text_field(wp_unslash($_POST['license_key'])) : ''];
        ...
        ?>
            ...
            <div class="appsero-license-settings appsero-license-section">
                <?php 
        $this->show_license_page_card_header($license);
        ?>
        ...
        echo wp_create_nonce($this->client->name);
        ?>">
        ...
        echo $this->get_input_license_value($action, $license);
        ?>"
                                    placeholder="<?php 
        echo esc_attr($this->client->__trans('Enter your license key to activate'));
        ?>" name="license_key"
                                    <?php 
        echo 'deactive' === $action ? 'readonly="readonly"' : '';
        ?>
                                />
                            </div>
                            <button type="submit" name="submit" class="<?php 
        echo 'deactive' === $action ? 'deactive-button' : '';
        ?>">
                                <?php 
        echo $action === 'active' ? $this->client->__trans('Activate License') : $this->client->__trans('Deactivate License');
        ?>
                            </button>
                        </div>
                    </form>
        ...
        ?>
            ...
            <?php 
        do_action('after_appsero_license_section');
        ?>
        </div>
        <?php 
    }
    ...
    private function show_active_license_info($license)
    {
        ...
        <div class="active-license-info">
            <div class="single-license-info">
                <h3><?php 
        $this->client->_etrans('Activations Remaining');
        ?></h3>
                <?php 
        if (empty($license['activation_limit'])) {
            ?>
                    <p><?php 
            $this->client->_etrans('Unlimited');
            ?></p>
                <?php 
        } else {
            ?>
        ...
        ?>
            ...
            <div class="single-license-info">
                <h3><?php 
        $this->client->_etrans('Expires in');
        ?></h3>
                <?php 
    ...
    private function show_license_page_card_header($license)
    {
        ...
        <div class="appsero-license-title">
            ...
            <span><?php 
        echo $this->client->__trans('Activate License');
        ?></span>
        ...
        if ($license && $license['key']) {
            ...
            ?>">
                <button type="submit" name="submit" class="appsero-license-refresh-button">
                    ...
                    <?php 
            echo $this->client->__trans('Refresh License');
            ?>
                </button>
            </form>
            <?php 
    ...
    private function active_client_license($license_key)
    {
        ...
        $data = ['key' => $license_key, 'status' => 'activate', 'remaining' => $response['remaining'], 'activation_limit' => $response['activation_limit'], 'expiry_days' => $response['expiry_days'], 'title' => $response['title'], 'source_id' => $response['source_identifier'], 'recurring' => $response['recurring']];

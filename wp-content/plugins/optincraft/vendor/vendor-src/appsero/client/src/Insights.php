...
class Insights
{
    ...
    public function __construct($client, $name = null, $file = null)
    {
        ...
        if (is_object($client) && is_a($client, 'OptinCraft\Appsero\Client')) {
            $this->client = $client;
        }
    }
    /**
    ...
    {
        $inaccessible_pages = array('/wp-admin/update.php');
    ...
    {
        global $wpdb;
        return (int) $wpdb->get_var($wpdb->prepare("SELECT count(ID) FROM {$wpdb->posts} WHERE post_type = %s and post_status = %s", $post_type, 'publish'));
    }
    /**
    ...
    private function get_all_plugins()
    {
        ...
        foreach ($plugins as $k => $v) {
            $formatted = array('name' => wp_strip_all_tags($v['Name']), 'version' => wp_strip_all_tags($v['Version']), 'author' => wp_strip_all_tags($v['Author']));
            if (isset($v['Network'])) {
    ...
    {
        $schedules['weekly'] = array('interval' => DAY_IN_SECONDS * 7, 'display' => __('Once Weekly', 'appsero'));
        return $schedules;
    }
    /**
    ...
    public function deactivate_scripts()
    {
        ...
        <div class="wd-dr-modal" id="<?php 
        echo $this->client->slug;
        ?>-wd-dr-modal">
            <div class="wd-dr-modal-wrap">
                <div class="wd-dr-modal-header">
                    <h3> <?php 
        $this->client->_etrans('Goodbyes are always hard. If you have a moment, please let us know how we can improve.');
        ?> </h3>
                </div>
        ...
        foreach ($reasons as $reason) {
            ?>
                            <li data-placeholder="<?php 
            echo esc_attr($reason['placeholder']);
            ?>">
                                <label>
                                    <input type="radio" name="selected-reason" value="<?php 
            echo $reason['id'];
            ?>">
                                    <div class="wd-de-reason-icon"><?php 
            echo $reason['icon'];
            ?></div>
                                    <div class="wd-de-reason-text"><?php 
            echo $reason['text'];
            ?></div>
                                </label>
                            </li>
        ...
        if ($custom_reasons && is_array($custom_reasons)) {
            ?>
                        <ul class="wd-de-reasons wd-de-others-reasons">
                            <?php 
            foreach ($custom_reasons as $reason) {
                ?>
                                <li data-placeholder="<?php 
                echo esc_attr($reason['placeholder']);
        ...
        ?>
                ...
                <div class="wd-dr-modal-footer">
                    <a href="#" class="dont-bother-me wd-dr-button-secondary"><?php 
        $this->client->_etrans('Skip & Deactivate');
        ?></a>
                    <button class="wd-dr-button-secondary wd-dr-cancel-modal"><?php 
        $this->client->_etrans('Cancel');
        ?></button>
                    <button class="wd-dr-submit-modal"><?php 
        $this->client->_etrans('Submit & Deactivate');
        ?></button>
                </div>
            </div>
        </div>
        ...
        ?>-wd-dr-modal');
                    ...
                    // Open modal
                    $('#the-list').on('click', 'a.<?php 
        echo $this->client->slug;
        ?>-deactivate-link', function(e) {
                        e.preventDefault();
                    ...
                    modal.on('click', 'button.wd-dr-submit-modal', function(e) {
                        ...
                        $.ajax({
                            ...
                            data: {
                                nonce: '<?php 
        echo wp_create_nonce('appsero-security-nonce');
        ?>',
                                action: '<?php 
        echo $this->client->slug;
        ?>_submit-uninstall-reason',
    ...
    {
        $skipped = get_option($this->client->slug . '_tracking_skipped');
        $data = ['hash' => $this->client->hash, 'previously_skipped' => \false];
        if ($skipped === 'yes') {
            $data['previously_skipped'] = \true;
        } else {
